...
graph TD
  ...
  O --> MR[Multiple Realizability]
  O --> DC[Downward Causation]
  O --> H[Holism vs Atomism]

  Ep --> MP[Methodological Pluralism]
  Ep --> ME[Model Ensembles]
  Ep --> CE[Computational Epistemology]

  M --> IC[Inter-level Causation]
  M --> CB[Constraint-based Explanation]
  M --> SD[Scale-dependent Phenomena]

  N --> CT[Critical Transitions]
  N --> SC[Sensitive Dependence]
  N --> FD[Fractal Dimensions]

  P --> HC[Historical Contingency]
  P --> BE[Butterfly Effect]
  P --> AT[Attractor Dynamics]
```
