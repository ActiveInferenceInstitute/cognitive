...
def create_cultural_analysis(requests_data: List[Dict], output_dir: str):
    ...
    try:
        ...
        print("Note: gensim not installed. Skipping topic modeling.")
    ...
    try:
        ...
        print("Note: textblob not installed. Skipping sentiment analysis.")
    ...
    # Create language complexity visualization
    ...
    for i, (metric, title) in enumerate(zip(metrics, titles)):
        ax = [ax1, ax2, ax3][i]
        positions = range(1, len(unique_agents) + 1)
        ...
        if i == 0:
            handles = [plt.Rectangle((0,0),1,1, fc=agent_color_map[agent]) 
                      for agent in unique_agents]
            ax.legend(handles, unique_agents, 
                     loc='upper right', 
