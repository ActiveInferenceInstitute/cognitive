...
class MatrixPlotter:
    ...
    def plot_heatmap(
        ...
        **kwargs,
        ...
        sns.heatmap(
            matrix, ax=ax, cmap=cmap or self.style_config.get("colormap", "viridis"), **kwargs
        )
    ...
    def plot_multi_heatmap(
        ...
        for i, ax in enumerate(axes):
            sns.heatmap(tensor[i], cmap=colormap, annot=True, fmt=".2f", ax=ax, cbar=True)
    ...
    def plot_belief_evolution(
        self, beliefs: np.ndarray, ax: plt.Axes | None = None, state_labels: list | None = None
    ) -> plt.Axes:
...
class StateSpacePlotter:
    ...
    def plot_belief_evolution(
        self, beliefs: np.ndarray, title: str, state_labels: list[str], save_name: str | None = None
    ) -> plt.Figure:
        """Plot belief evolution over time."""
        ...
        for i in range(beliefs.shape[1]):
            ax.plot(time_steps, beliefs[:, i], label=state_labels[i], marker="o")
    ...
    def plot_free_energy_landscape(
        self, free_energy: np.ndarray, title: str, save_name: str | None = None
    ) -> plt.Figure:
...
class NetworkPlotter:
    ...
    def plot_message_network_dynamics(
        self,
        ...
        for edge in edges:
            is_active = any(
                msg["source"] == edge[0] and msg["target"] == edge[1] for msg in messages
            )
