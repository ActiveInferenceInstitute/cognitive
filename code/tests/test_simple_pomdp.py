...
def config_path(tmp_path):
    ...
    config = {
        "model": {"name": "SimplePOMDP", "description": "Test POMDP", "version": "0.1.0"},
        "state_space": {"num_states": 3, "state_labels": ["S1", "S2", "S3"], "initial_state": 0},
        "observation_space": {"num_observations": 2, "observation_labels": ["O1", "O2"]},
        "action_space": {"num_actions": 2, "action_labels": ["A1", "A2"]},
        "matrices": {
            "A_matrix": {
                "shape": [2, 3],
        ...
        "visualization": {
            ...
            "formats": ["png"],
            "dpi": 300,
            "style": {"colormap_2d": "YlOrRd", "colormap_3d": "viridis", "figure_size": [10, 8]},
        },
    }
...
def model(output_dir):
    ...
    config = {
        "model": {"name": "test_pomdp", "description": "Test POMDP model", "version": "0.1"},
        "state_space": {"num_states": 3, "state_labels": ["S1", "S2", "S3"], "initial_state": 0},
        "observation_space": {"num_observations": 2, "observation_labels": ["O1", "O2"]},
        "action_space": {"num_actions": 2, "action_labels": ["A1", "A2"]},
        "matrices": {
...
def test_matrix_properties(model):
    ...
    assert np.all(model.config["action_space"]["num_actions"] > model.E)
...
def test_expected_free_energy(model):
    ...
    total_efe, epistemic, pragmatic = compute_expected_free_energy(
        A=model.A, B=model.B, C=model.C, beliefs=model.state.beliefs, action=0
    )
    ...
    # EFE is pragmatic value minus epistemic information gain.
    assert np.allclose(total_efe, pragmatic - epistemic)
...
def test_temporal_consistency(model):
    ...
    n_steps = 10
    history = {"states": [], "observations": [], "actions": [], "beliefs": [], "free_energies": []}
...
def test_learning_rate_sensitivity(model):
    ...
    for lr in learning_rates:
        ...
        for _ in range(n_steps):
            ...
            model._update_beliefs(initial_observation, 0)
    ...
    assert belief_changes[0] < belief_changes[1], (
        "Higher learning rates should lead to larger belief changes"
    )
